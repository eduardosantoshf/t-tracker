...
function showPosition(position) {
    ...
    fetch('http://localhost:8080/rider/location/' + lat + "/" + long, { headers: { 'Content-Type': 'application/json', 'Authorization': "Bearer " + getCookie("sessionKey") }, method: 'post' }).then(data => {
        if (data.status == 200) {
            data = data.json();
